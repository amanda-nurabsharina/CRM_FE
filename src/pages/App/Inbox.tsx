...
import { MessageSquare, Send, Phone, MapPin, Clock, Tag, UserCheck, ShieldCheck } from "lucide-react";
...
export const InboxPage: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex overflow-hidden bg-zinc-950 text-zinc-100">
      {/* Left List of Conversations */}
      <div className="w-80 border-r border-zinc-800/80 flex flex-col bg-zinc-900/30">
        <div className="p-4 border-b border-zinc-800/80">
          <h2 className="text-base font-extrabold flex items-center gap-2">
            <MessageSquare className="h-5 w-5 text-teal-400" />
            <span>WhatsApp Inbox</span>
          </h2>
          <p className="text-xs text-zinc-400 mt-1">Multi-branch customer chats</p>
        </div>
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 text-left transition-colors flex items-start gap-3 ${
                    isSelected ? "bg-teal-500/10 border-l-4 border-teal-400" : "hover:bg-zinc-900/60"
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-bold truncate text-zinc-100">{conv.lead?.customer_name || "Pelanggan Baru"}</p>
                      <span className="text-[10px] text-zinc-500">
                        {new Date(conv.last_message_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                      </span>
                    </div>
                    <p className="text-[11px] text-zinc-400 truncate mt-0.5">{conv.lead?.phone_number}</p>
      ...
      {activeConv ? (
        <div className="flex-1 flex flex-col bg-zinc-950">
          {/* Chat Header */}
          <div className="h-16 px-6 border-b border-zinc-800/80 flex items-center justify-between bg-zinc-900/50">
            <div className="flex items-center gap-3">
              <div className="h-9 w-9 rounded-full bg-teal-500/20 border border-teal-500/40 text-teal-300 font-bold flex items-center justify-center text-xs">
                {activeConv.lead?.customer_name?.slice(0, 2).toUpperCase() || "WA"}
              </div>
              <div>
                <h3 className="text-sm font-bold text-zinc-100">{activeConv.lead?.customer_name || "WhatsApp Customer"}</h3>
          ...
          {/* Message Input Box */}
          <form onSubmit={handleSend} className="p-4 border-t border-zinc-800/80 bg-zinc-900/40 flex items-center gap-3">
            <input
              type="text"
              value={messageText}
              onChange={(e) => setMessageText(e.target.value)}
              placeholder="Ketik balasan resmi WhatsApp (Auto-saved to Lead Record)..."
              className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-2.5 text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-teal-500"
            />
            <button
              type="submit"
      ...
      {activeConv?.lead && (
        <div className="w-72 border-l border-zinc-800/80 bg-zinc-900/40 p-4 space-y-6 overflow-y-auto">
          <div>
            <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-3">Informasi Customer</h4>
            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1 border-b border-zinc-800/60">
                <span className="text-zinc-500">Nama</span>
                <span className="font-semibold text-zinc-200">{activeConv.lead.customer_name}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-zinc-800/60">
                <span className="text-zinc-500">Nomor WA</span>
                <span className="font-semibold text-zinc-200">{activeConv.lead.phone_number}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-zinc-800/60">
                <span className="text-zinc-500">Cabang</span>
                <span className="font-semibold text-teal-400">{activeConv.lead.branch?.name || "DGT Pusat"}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-zinc-800/60">
                <span className="text-zinc-500">Status Pipeline</span>
          ...
          <div>
            <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Internal Handover Note</h4>
            <textarea
              defaultValue={activeConv.lead.handover_note || ""}
              placeholder="Catatan serah terima antar admin cabang..."
              className="w-full h-24 bg-zinc-900 border border-zinc-800 rounded-xl p-2.5 text-xs text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-teal-500"
            />
          </div>
        </div>
      )}
