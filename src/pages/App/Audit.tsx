    ...
    <div className="p-6 space-y-6 bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100 min-h-screen transition-colors">
      ...
      <div className="bg-white dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm dark:shadow-none">
        <div className="p-4 border-b border-slate-200 dark:border-zinc-800/80 flex items-center justify-between">
          <span className="text-xs font-bold text-slate-700 dark:text-zinc-300">Aktivitas Terbaru</span>
          ...
          {logs.length === 0 ? (
            ...
            logs.map((log) => (
              <div key={log.id} className="p-4 hover:bg-slate-50 dark:hover:bg-zinc-900/80 transition-colors flex items-start gap-4">
                <div className="h-8 w-8 rounded-full bg-teal-500/10 border border-teal-500/30 text-teal-600 dark:text-teal-400 flex items-center justify-center shrink-0 mt-0.5">
                  <Activity className="h-4 w-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-900 dark:text-zinc-200">{log.action_type}</span>
                    <Badge variant="indigo" className="text-[10px] py-0 px-1.5">{log.entity_name}</Badge>
                  </div>
                  <p className="text-xs text-slate-500 dark:text-zinc-400 mt-1">
                    Entity ID: <span className="font-mono text-slate-700 dark:text-zinc-300">{log.entity_id}</span>
                  </p>
                </div>
