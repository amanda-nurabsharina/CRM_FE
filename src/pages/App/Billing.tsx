...
import { crmApi, Invoice } from "../../api/crmApi";
import { CreditCard, CheckCircle2, XCircle, Clock, ShieldCheck, FileText, Upload, AlertCircle } from "lucide-react";
  ...
  return (
    <div className="p-6 space-y-6 bg-zinc-950 text-zinc-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 gap-4">
        {invoices.length === 0 ? (
          <div className="p-12 text-center border border-zinc-800/80 rounded-2xl bg-zinc-900/30 text-zinc-500 text-xs">
          ...
          invoices.map((inv) => {
            ...
            return (
              <div key={inv.id} className="p-5 bg-zinc-900/50 border border-zinc-800 rounded-2xl space-y-4 hover:border-zinc-700 transition-all">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-zinc-800/60 pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-extrabold text-zinc-100">{inv.invoice_number}</h3>
                      <p className="text-xs text-zinc-400">Customer: {inv.lead?.customer_name || "Customer"}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className="text-sm font-extrabold text-teal-400">Rp {inv.total_amount.toLocaleString("id-ID")}</p>
                    </div>
                ...
                <div>
                  <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Termin Cicilan / Skema Pembayaran</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {inv.terms?.map((term) => (
                      <div key={term.id} className="p-3 bg-zinc-950 border border-zinc-800/80 rounded-xl space-y-2">
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-bold text-zinc-200">Termin {term.term_number}</span>
                          <span className="text-teal-400 font-semibold">Rp {term.amount.toLocaleString("id-ID")}</span>
                        </div>
                          ...
                          <div className="pt-2 border-t border-zinc-800/60">
                            {term.proofs.map((proof) => (
                              <div key={proof.id} className="flex items-center justify-between">
                                <span className="text-[10px] text-zinc-400">Bukti Transfer TF</span>
                                {proof.verification_status === "PENDING_PUSAT" ? (
                                  <button
                                    ...
                                    className="px-2 py-1 bg-amber-500/20 text-amber-300 border border-amber-500/40 rounded text-[10px] font-bold hover:bg-amber-500/30 flex items-center gap-1"
                                  >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-2 text-xs text-zinc-300">
              <p>Nominal Ditransfer: <span className="font-bold text-teal-400">Rp {selectedProof.amount.toLocaleString("id-ID")}</span></p>
              <div className="h-48 rounded-xl overflow-hidden bg-zinc-950 border border-zinc-800 flex items-center justify-center">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                disabled={verifyMutation.isPending}
                className="flex-1 py-2.5 bg-red-500/20 text-red-300 border border-red-500/40 hover:bg-red-500/30 font-bold rounded-xl text-xs flex items-center justify-center gap-2"
              >
